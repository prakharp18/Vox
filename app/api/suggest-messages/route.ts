...
export async function POST(req: Request) {
  try {
    ...
    const clickCount = parseInt(cookieStore.get('suggest_clicks')?.value || '0');

    if (clickCount >= 3) {
      return NextResponse.json(
        { message: "You've reached the suggestion limit. Please log in to get more!" },
        { status: 403 }
      );
    }
    ...
    // Update the click counter cookie
